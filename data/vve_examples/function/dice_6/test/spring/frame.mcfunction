#vve_examples:dice_6/test/spring/frame
...
# 设置销毁函数
data modify storage marker_control:io result.del_func set value "vve_examples:dice_6/_del"
execute as @e[tag=result,limit=1] run function marker_control:data/_store
tag @e[tag=result,limit=1] add entity_todel
scoreboard players set @e[tag=result,limit=1] killtime 50
execute as @e[tag=result,limit=1] run function vve_examples:dice_6/main