#vve_examples:slime_block/_iter_cpoints
# 遍历顶点作为碰撞点，进行介质探测
# 输出介质响应(各模块的临时对象)
...
scoreboard players operation temp_v int /= 2 int
scoreboard players operation vy int += temp_v int
...
execute if score impulse_response int matches 1 run function vve:object/_receive_impulse_f
